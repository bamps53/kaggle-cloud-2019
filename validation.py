...
def validation(config_file_seg):
    ...
    all_dice = np.zeros((4, len(label_thresholds), len(mask_thresholds), len(min_sizes)))
    count = 0
    ...
    with torch.no_grad():
        for i, (batch_images, batch_masks) in enumerate(tqdm(validloader)):
            ...
            batch_labels = torch.nn.functional.adaptive_max_pool2d(torch.sigmoid(torch.Tensor(batch_preds)),1).view(batch_preds.shape[0], -1)

            batch_masks = batch_masks.cpu().numpy()
            batch_labels = batch_labels.cpu().numpy()

            batch_masks = resize_batch_images(batch_masks, SUB_HEIGHT, SUB_WIDTH)
            batch_preds = resize_batch_images(batch_preds, SUB_HEIGHT, SUB_WIDTH)
